...
def execute(filters=None):
	...
	for emp in employees:
		...
		annual_leave_balance = get_leave_balance_on(emp.name, "Annual Leave", filters.get('to_date')) or 0
		sick_leave_balance_full = get_leave_balance_on(emp.name, "Sick Leave - Full Paid", filters.get('to_date')) or 0
		sick_leave_balance_half = get_leave_balance_on(emp.name, "Sick Leave - Half Paid", filters.get('to_date')) or 0
		leave_accrual = flt(accrual_total) / 30 * flt(annual_leave_balance)

		row += [annual_leave_balance, sick_leave_balance_full, sick_leave_balance_half, leave_accrual]

		working_years = flt(flt((getdate(filters.get('to_date')) - getdate(emp.date_of_joining)).days) / 365)
		indemnity_accrual = 0

		if emp.employee_category == 'Non-Bahraini':
			if working_years >= 3:
				indemnity_accrual = flt(accrual_total) * flt(working_years - 1.5)
			else:
				indemnity_accrual = flt((working_years * accrual_total) / 2)

		row += [working_years, indemnity_accrual]
...
def get_columns(employees):
	...
	columns = columns + [
		_("Annual Leave Balance") + ":Float:130",
		_("Sick Leave Balance - Full Paid") + ":Float:140",
		_("Sick Leave Balance - Half Paid") + ":Float:140",
		_("Leave Accrual") + ":Currency:120",
		_("Working Years") + ":Float:120",
		_("Indemnity Accrual") + ":Currency:120"
	]
