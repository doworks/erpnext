...
def execute(filters=None):
	...
	leave_list = None
	leave_types = frappe.db.sql("""select name from `tabLeave Type` order by name""", as_list=True)
	leave_list = [d[0] for d in leave_types]

	days_diff = frappe.utils.date_diff(filters.to_date, filters.from_date) + 1
	...
	if filters.group_by:
		...
		for parameter in group_by_parameters:
			...
			if (att_map_set & emp_map_set):
				parameter_row = ["<b>"+ parameter + "</b>"] + ['' for day in range(days_diff)]
...
def get_chart_data(emp_att_map, days):
	...
	for idx, day in enumerate(days, start=0):
		...
		datasets[0]["values"].append(total_leave_on_day)
...
def add_data(employee_map, att_map, filters, holiday_map, conditions, default_holiday_list, leave_list=None):
	...
	for emp in employee_map:
		...
		for day in range(days_diff):
			...
			abbr = ''.join([x[0] for x in status.split()]) if status else ''
...
def get_columns(filters):
	...
	days_diff = frappe.utils.date_diff(filters.to_date, filters.from_date) + 1
	for day in range(days_diff):
		date = frappe.utils.add_to_date(filters.from_date, days=day)
	...
	#if filters.summarized_view:
	for leave in frappe.db.sql("""select name from `tabLeave Type` order by name""", as_list=True):
		columns += [_(leave[0]) + ":Float:120"]
	return columns, days

def get_attendance_list(conditions, filters):
	attendance_list = frappe.db.sql("""SELECT employee, day(attendance_date) as day_of_month, leave_type, status 
		FROM tabAttendance where docstatus = 1 and status = "On Leave" %s order by employee, attendance_date""" %
		conditions, filters, as_dict=1)

	if not attendance_list:
		msgprint(_("No leave record found"), alert=True, indicator="orange")
	...
	for d in holiday_list:
		if d:
			holiday_map.setdefault(d, frappe.db.sql('''select day(holiday_date), weekly_off from `tabHoliday`
				where parent=%s and holiday_date >= %s and holiday_date <= %s''', (d, filters.from_date, filters.to_date)))
